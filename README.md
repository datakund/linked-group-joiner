...
A python library that uses browser automation to login & join groups on linkedin
...
```sh
pip install linkedin-group-joiner
```

### Import

```javascript
from linkedin-group-joiner import *
```

### Login

To join linkedin groups first we will login to linkedin. There are two ways of login:-
* Credentials
* Cookies
...
To join group on linkedin we use ``join_group`` method.
It requires **group_link** as input parameter.

```javascript
linkedin.join_group(group_link="group_link_here")
```
